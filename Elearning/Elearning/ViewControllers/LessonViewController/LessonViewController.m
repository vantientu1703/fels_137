...
- (void)viewDidLoad {
    ...
    [self getLessonWithCategoryID:self.categoryItem.ID authToken:USER_TOKEN];
}
...
- (void)turnOnAlertWithMessage:(NSString *)message {
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:RELOAD_ACT_CONTROLLER style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        _pagingAtScrollView = 0;
        [self getLessonWithCategoryID:self.categoryItem.ID authToken:USER_TOKEN];
    }];
...
- (void)loadData {
    ...
    if (self.arrWords.count > 0) {
        [self.arrWords enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            WordItem *wordItem = [DBUtil dbItemToWordItem:self.arrWords[idx]];
    ...
    for (NSInteger i = 0; i < self.arrAnswers.count; i++) {
        NSString *title = self.arrAnswers[i][@"content"];
        [self setTitleForButtonNoSelect:title withTag:i + 1];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.arrAnswers.count > 0) {
        [self.arrAnswers enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *title = self.arrAnswers[idx][@"content"];
...
- (IBAction)btnAnswerPress:(id)sender {
    ...
    if ([sender isMemberOfClass:[UIButton class]]) {
        ...
        if (self.arrAnswers.count > 0) {
            answerItem = [DBUtil dbAnswerItem:self.arrAnswers[tag - 1]];
        }
        ...
        [lessCategoryManager updateLessonWithAuthToken:USER_TOKEN
